...
const defaultProps = {
  ...
  describe('TaskHeaderComponent – filtering', () => {
    ...
    it('opens priority dropdown on funnel click', () => {
      ...
      // Click the button then force a tick for OnPush change detection
      cy.get('.priority-filter-wrap ion-button').click();
      cy.tick(50);
    ...
    it('closes dropdown when clicking outside', () => {
      ...
      // Click on the component's host but outside the dropdown
      // HostListener fires reliably on the mounted component element
      cy.get('app-task-header').click(0, 0, { force: true });
      cy.tick(50);
